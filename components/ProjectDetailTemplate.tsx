    ...
    <main className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8 lg:px-12">
      <article>
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-16 text-xs font-semibold uppercase tracking-[0.18em] text-stone-500">{project.category}</p>
          <h1 className="mt-5 text-5xl font-semibold leading-[0.98] text-stone-950 sm:text-6xl md:text-7xl">
            {project.title}
          </h1>
          <p className="mt-7 text-xl leading-9 text-stone-600 sm:text-2xl">{project.description}</p>
          <dl className="mt-10 grid gap-5 border-y border-stone-300/80 py-6 text-sm sm:grid-cols-3">
            ...
            <div>
              ...
              <dd className="mt-2 font-medium text-stone-950">{project.category}</dd>
            </div>
            <div>
        ...
        <section className="mx-auto mt-16 max-w-3xl">
          {sections.map((section) => (
            <section key={section.key} className="border-t border-stone-300/80 py-9">
              ...
              <p className="mt-5 text-xl leading-9 text-stone-700">{project[section.key]}</p>
            </section>
          ))}
        </section>
        ...
        <section className="mx-auto mt-8 max-w-5xl">
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-2">
            <div className="h-80 overflow-hidden bg-white shadow-[0_18px_46px_rgba(67,64,58,0.12)]">
